...
from voice_conversion.cycle_began import CycleBeGAN
from voice_conversion.cyclegan import CycleGAN

from voice_conversion.Utils.utils import *
...
def converting(direction=direction, model_dir=model_dir, test_dir=test_dir, sr=sr, n_features=n_features, frame_period=frame_period) :
    ...
    outputs_dir = "./voice_conversion/sample"
    ...
    model.load(os.path.join("./voice_conversion/model"))

    mcep = np.load(os.path.join("./voice_conversion/data/", 'mcep.npz'))
    mcep_mean_A = mcep['A_mean']
    mcep_std_A = mcep['A_std']
    mcep_mean_B = mcep['B_mean']
    mcep_std_B = mcep['B_std']

    logf0s = np.load(os.path.join("./voice_conversion/data/", 'logf0s.npz'))
