...
from text_conversion.cycle_began import CycleBeGAN
from text_conversion.cyclegan import CycleGAN

from text_conversion.Utils.utils import *
from text_conversion.config import *


def converting(direction=direction, model_dir=model_dir, test_dir=test_dir, sr=sr, n_features=n_features, frame_period=frame_period):

    outputs_dir = "./text_conversion/sample"
    ...
    model.load(os.path.join("./text_conversion/model"))

    mcep = np.load(os.path.join("./text_conversion/data/", 'mcep.npz'))
    mcep_mean_A = mcep['A_mean']
    mcep_std_A = mcep['A_std']
    mcep_mean_B = mcep['B_mean']
    mcep_std_B = mcep['B_std']

    logf0s = np.load(os.path.join("./text_conversion/data/", 'logf0s.npz'))
